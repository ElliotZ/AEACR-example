...
public class 大舞前2G优化 : ISlotResolver
{
    ...
    private class 落幕舞 : ISlotSequence
    {
        public List<Action<Slot>> Sequence => [(Slot slot) => slot.Add(Data.Spells.落幕舞.GetSpell())];
    }
    ...
    public void Build(Slot slot)
    {
        ...
        if (幻扇 == 4) slot.AppendSequence(new 落幕舞扇舞序()); //进大舞时没有剑舞落幕舞，扇舞有4个时泄掉一个
        else slot.AppendSequence(new 落幕舞());
    }
}