...
public class 灵橡 : ISlotResolver
{
    ...
    public int Check()
    {
        ...
        if (!Data.Spells.灵橡清汁.GetSpell().IsUnlock()) return -2;
        if (SgeHelper.蓝豆 <= 0) return -2;
        ...
        var list = PartyHelper.CastableAlliesWithin30
            .Concat([Core.Me])
            .Where(ally => ally.CurrentHpPercent() <= SgeSettings.Instance.灵橡阈值 / 100f)
            .ToList();
    ...
    {
        slot.Add(new Spell(Data.Spells.灵橡清汁, _target));
    }
}